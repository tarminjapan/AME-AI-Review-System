...
【 Gate 1: ローカルコミット時 (pre-commit) 】
 [コード変更] ──► git commit ──► [静的解析 (ruff/mypy/semgrep)] ──► PASS ──► [ローカルAIレビュー]
                                       ...
                                       (エラー)                                    (指摘あり)
                                           ▼                                          ▼
                                       ブロック                                   ブロック (※LOW/INFOのみ連続 2 回でエスケープ可。重大度によらず precommit_max_reviews 回でエスケープ可)
...
4. **エスケープハッチ (無限ループ回避)**: 2 つの独立した escape 機構がある（Issue #134）。
   - `LOW` レベル以下の指摘のみが**固定 2 回**連続した場合、コミットが許可（PASS）される。
   - 総レビュー回数が `precommit_max_reviews`
     回（既定 3 回）に達した場合、blocking 指摘（MIDDLE 以上）が残っていてもコミットが許可（PASS）される。このとき警告が出力される。
...
2. 返信は**インライン返信**で投稿する。PR 本文コメント（issue_comment）へのメンションでは LGTM 返信は行われない（再レビューは
   `/request-review` を使用する）。
   - **1 返信 = 1 LGTM**: 各インライン返信に対しては、**そのスレッドだけ**に LGTM / 追加指摘が返る。

### Step 2-5: AI による再評価と LGTM 返信の確認

1. インライン返信を検知すると、AI レビュアーが自動的に最新の diff を検証し、**そのスレッドにだけ**返信する。
...
> [!IMPORTANT] **レビュー回数上限（Issue #129）** 1 つの PR に対して、`/request-review`
> によるレビューは **`pr_max_reviews` 回（既定 3 回）**
> が上限です。上限に達するとレビューはスキップされ、PR に一度だけ「レビュー回数上限に達しました」という通知コメントが投稿されて Gate
> 2 は終了します。上限は `config.json` の `pr_max_reviews` で変更できます。
